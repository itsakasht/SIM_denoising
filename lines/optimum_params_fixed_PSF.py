...
os.makedirs(os.path.join(current_dir, "results/plots/optimum_params_fixed_psf"), exist_ok=True)
...
#          ]  # PSF for RL filter
rl_psf = op.incoherent_psf(psfsize=25, wavelength=480, NA=1.2, pixelsize=20)  # PSF for RL filter


balance = np.array([0.1, 1, 2, 5, 10, 20, 40]) # Wiener filtering
weight = np.array([0.01, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1]) # TV filtering
iterations = np.array([1, 2, 3, 4, 5, 10, 20]) # RL filtering
sigma_psd = np.array([0.01, 0.1, 0.5, 1, 2, 4, 6, 8, 10]) # BM3D filtering
...
for i in range(len(SNR)):
    for j in range(len(noise_types)):
        ...
        ax1.plot(balance, w_psnr[len(balance):2*len(balance)], label=f'Gaussian PSF 5', color=cool_color[1])
        ax1.plot(balance, w_psnr[2*len(balance):3*len(balance)], label=f'Gaussian PSF 7', color=cool_color[2])
        ax1.plot(balance, w_psnr[3*len(balance):4*len(balance)], label=f'Gaussian PSF 9', color=cool_color[3])
        ax1.plot(balance, w_psnr[4*len(balance):5*len(balance)], label=f'Gaussian PSF 11', color=warm_color[0])
        ax1.plot(balance, w_psnr[5*len(balance):6*len(balance)], label=f'Gaussian PSF 13', color=warm_color[1])
        ax1.plot(balance, w_psnr[6*len(balance):7*len(balance)], label=f'Gaussian PSF 15', color=warm_color[2])
        ax1.plot(balance, w_psnr[7*len(balance):], label=f'Gaussian PSF 17', color=warm_color[3])
        ...
        ax2.plot(balance, w_ssim[len(balance):2*len(balance)], label=f'Gaussian PSF 5', color=cool_color[1])
        ax2.plot(balance, w_ssim[2*len(balance):3*len(balance)], label=f'Gaussian PSF 7', color=cool_color[2])
        ax2.plot(balance, w_ssim[3*len(balance):4*len(balance)], label=f'Gaussian PSF 9', color=cool_color[3])
        ax2.plot(balance, w_ssim[4*len(balance):5*len(balance)], label=f'Gaussian PSF 11', color=warm_color[0])
        ax2.plot(balance, w_ssim[5*len(balance):6*len(balance)], label=f'Gaussian PSF 13', color=warm_color[1])
        ax2.plot(balance, w_ssim[6*len(balance):7*len(balance)], label=f'Gaussian PSF 15', color=warm_color[2])
        ax2.plot(balance, w_ssim[7*len(balance):], label=f'Gaussian PSF 17', color=warm_color[3])
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(current_dir, f"results/plots/optimum_params_fixed_psf/wiener_snr{SNR[i]}_{noise_types[j]}_balance.png"))
        plt.close(fig)
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
        ax1.plot(weight, t_psnr, label='PSNR', color=cool_color[0])
        ax1.axhline(y=noisy_psnr, color='black', linestyle='--', label='Noisy PSNR', alpha=0.8)
        ax1.set_xscale('log')
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(current_dir, f"results/plots/optimum_params_fixed_psf/tv_snr{SNR[i]}_{noise_types[j]}_weight.png"))
        plt.close(fig)
        

        # Richardson Lucy filtering
        r_psnr = np.zeros(len(iterations))
        r_ssim = np.zeros(len(iterations))
        
        for l in range(1):
            for k in range(len(iterations)):
                rl0 = ski.restoration.richardson_lucy(noisy[0, i, j], rl_psf, num_iter=iterations[k], filter_epsilon=1e-10)
                rl1 = ski.restoration.richardson_lucy(noisy[1, i, j], rl_psf, num_iter=iterations[k], filter_epsilon=1e-10)
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 6))
        ax1.plot(iterations, r_psnr[0:len(iterations)], label=f'Incoherent PSF 480nm', color=cool_color[0])

        ax1.axhline(y=noisy_psnr, color='black', linestyle='--', label='Noisy PSNR', alpha=0.8)
        ax1.set_xlabel('Number of iterations')
        ax1.set_xlim(left=0, right=iterations[-1])
        ax1.set_ylabel('PSNR')
        ax1.set_ylim(0, 60)
        ax1.legend(loc='upper left')
        ax1.tick_params(axis='y')

        ax2.plot(iterations, r_ssim[0:len(iterations)], label=f'Incoherent PSF 480nm', color=warm_color[0])
        
        ax2.axhline(y=noisy_ssim, color='black', linestyle='--', label='Noisy SSIM', alpha=0.8)
        ax2.set_xlabel('Number of iterations')
        ax2.set_xlim(left=0, right=iterations[-1])
        ax2.set_ylabel('SSIM')
        ax2.set_ylim(0, 1.2)
        ax2.legend(loc='upper right')
        ax2.tick_params(axis='y')
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(current_dir, f"results/plots/optimum_params_fixed_psf/rl_snr{SNR[i]}_{noise_types[j]}_iterations.png"))
        plt.close(fig)
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(current_dir, f"results/plots/optimum_params_fixed_psf/bm3d_snr{SNR[i]}_{noise_types[j]}_sigma.png"))
        plt.close(fig)
        ...
        if number == 0:
            w_best_psf = f'Gaussian 3'
        elif number == 1:
            w_best_psf = f'Gaussian 5'
        elif number == 2:
            w_best_psf = f'Gaussian 7'
        elif number == 3:
            w_best_psf = f'Gaussian 9'
        elif number == 4:
            w_best_psf = f'Gaussian 11'
        elif number == 5:
            w_best_psf = f'Gaussian 13'
        elif number == 6:
            w_best_psf = f'Gaussian 15'
        elif number == 7:
...
df.to_csv(os.path.join(current_dir, "results/plots/optimum_params_fixed_psf/optimum_filter_parameters.csv"), index=False)
